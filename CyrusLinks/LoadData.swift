...
func processAppData(rows: Dictionary<String, [GoogleSheetCell]>!) {
    ...
    for (key, cells) in rows {
        ...
        let matchedCategoryIndex = appData.firstIndex(where: { $0.name == categoryCell.data })
        ...
        if (matchedCategoryIndex == nil) {
            ...
            appData.append(newCategory)
            
            categoryIndex = appData.count - 1
        } else {
        ...
        let matchedSubCategoryIndex = appData[categoryIndex].subCategories.firstIndex(where: { $0.name == subCategoryCell.data })
        ...
        if (matchedSubCategoryIndex == nil) {
            ...
            appData[categoryIndex].subCategories.append(newSubCategory)

            subCategoryIndex = appData[categoryIndex].subCategories.count - 1
        } else {
        ...
        let linkUrl = linkUrlCell == nil ? (
            getDefaultGoogleUrl(subCategoryCell: subCategoryCell, linkNameCell: linkNameCell)
        ) : (
            linkUrlCell!.data
        )
        
        appData[categoryIndex].subCategories[subCategoryIndex].links.append(
